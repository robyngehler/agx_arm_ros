...
Typical solo workflow:
    ...
    # 3) drive a pose (one-shot)
    ros2 run agx_arm_ctrl omnihand_exerciser --side right --gesture fist

    # cycle a few poses, then open and stop
    ros2 run agx_arm_ctrl omnihand_exerciser --side right --sequence open,fist,open --stop

The poses come from the single source of truth in
``config/omnihand_gestures.yaml`` (derived from the OmniHand SDK
``demo_set_motion.py``). They are calibrated for the right hand; the bridge
clamps every target to the selected side's joint limits, so a right-tuned preset
on a left hand is safe but may not look identical. ``open`` is the flat open palm
(thumb rolled out and spread) and the safe default; ``zero`` is the all-zeros
motor reference, whose thumb rolls in across the palm and looks bent.
"""
...
# Gesture NAMES come from the single source of truth in
# config/omnihand_gestures.yaml; they are identical for both sides. The per-side
# joint VALUES are resolved at runtime (right = canonical, left = mirrored) via
# resolve_gesture_presets(side), so argparse choices stay side-agnostic while the
# published vectors match the selected hand.
GESTURE_NAMES: list[str] = sorted(load_gesture_presets())
...
class OmniHandExerciser(Node):
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.args = args
        self.joint_names = build_joint_names(args.side)
        self.gestures = resolve_gesture_presets(args.side)
        self.publisher = self.create_publisher(JointState, args.topic, 10)
        self.get_logger().info(
            f"Exercising side={args.side} on topic '{args.topic}' "
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.list:
        print("Available OmniHand poses:")
    ...
    if args.sequence:
        ...
        if unknown:
            raise SystemExit(f"unknown pose(s): {', '.join(unknown)}")
    elif args.gesture:
        poses = [args.gesture]
    elif not args.stop:
