...
- **a freshness stamp is only as fine-grained as whatever sets it.** `feedback/joint_states` carries the receive time of the last CAN frame to touch the driver's cache, and a complete joint update is four position frames covering joint pairs — so the stamp advances while the positions need not, and a stall can be **per joint** while the read is genuinely new for the rest of the arm. Ask what advances a timestamp before trusting it as "new data", and fix the artefact at the granularity it occurs at: refusing an unchanged read only catches the case where every joint stalled at once. Teach capture refuses unchanged reads *and* spreads a single joint's catch-up back over its hold, capped at 0.1 s so a real dwell is not ramped. Duration separates a stall from a dwell; speed does not — the measured stall implied 1.30 rad/s, under the limit, and still cost 9.86 rad/s²
- **a threshold tells you a sample is unusual, never why.** A teach recording is back-driven by hand, and a hand can move a joint faster than it can be commanded (3.93 rad/s is a setpoint limit, not a mechanical bound), so an over-limit sample is not proof of bad data. The shape is what discriminates: an isolated step whose neighbours are near zero is a cache catching up; a run of consecutive large steps is a motion really taught that fast — 11 of 14 over-limit samples across two duo takes were runs. Report what was measured and what it costs; leave the cause to whatever can actually establish it
- **an optimisation is only as good as the property it assumes.** The scale search for `speed_scale` assumed duration falls monotonically as the limits rise; it does not, because `_run_totg` corrects its limits against the peak it samples and that correction moves with the blend radii. A bisection then converges onto a local branch and reports its own dead end as the hardware's — 1.13x on a path that runs at 1.38x. Check the assumption against a trace before trusting a search, and keep the invariant that matters: seeding the same search from an estimate saved 40% of its time and returned plans *faster than requested*, which is the one thing it must never do
...
- **a recorded replay carries velocities, on every dispatch path.** The MIT trajectory buffer reads a missing velocity as a commanded zero, so `kd·(0 − q̇)` brakes against the motion the position term is asking for. The teach path and the coordinator both build a `JointTrajectory` for that one consumer; only the teach path knew it, and the production path measured `|v_des − dp/dt|` 0.224 rad/s against 0.004. **Two dispatch paths to one controller drift unless what they share is code, not a convention** — check the other path before calling a replay defect fixed. Catalogue waypoints are likewise chosen by chord error, not by even sample index, because a sparse budget spent by the clock lands on dwells (`docs/sprint_refactor/reference/teach_replay_timebase.md`)
- **a width or a window belongs to the time base of whatever it describes.** `tempo_scale` reconstructed the recording in *replay* seconds, so asking for a faster replay also filtered more of the taught path: the normalised path moved 0.126 rad at 3x and 0.027 rad at 0.5x, and commanded acceleration fell by less than tempo² because the shape itself had changed. A filter width is a property of the recording and applies in taught time; an output grid step is a property of the controller and applies in replay time. Reconstruct in the source's base and scale the axis last — the clock is then the whole of the operation, and every ratio in the recording survives it
- **two knobs that scale the same quantity multiply, and neither one reports it.** `velocity_scaling` stretched a recorded action's taught times before the playback mode saw them, so `tempo_scale: 0.5` under `velocity_scaling: 0.5` runs at a quarter speed with both numbers reading 0.5. Where a newer control supersedes an older one, refuse the pair rather than composing them, and leave the old one working alone so existing config migrates instead of breaking
- **a limit check may only refuse where something was requested, and what it suggests has to be usable.** `tempo_scale` carries a speed request, so a tempo past the joint limits is refused, naming the joint and the largest tempo that would pass; `as_recorded` and `smooth` reproduce whatever was taught and can only report, because a hand back-driving a joint can exceed a setpoint limit. A stand-in limit cannot carry a rejection at all — the `2.5 · v_max` acceleration is reported and never refused. And a suggested value is extrapolated, so it gets margin before it is printed: the exact boundary came back refused by the check that produced it
- **matching a shape is not matching an identity.** A left-arm recording has exactly the right number of joints for the right arm and would replay a mirrored path onto it, so a taught trajectory is checked against the group's ordered joint *names* before it is commanded. Where the names cannot establish it — a single-arm teach recording stores them unprefixed, which fits either arm — say so rather than accepting or refusing silently, and give the producer a way to record what is missing
