...
- **a repeated sample is not data, and the stored file cannot tell you which it was.** A still arm and a missing update look identical once written. Do not produce them: where a source has its own cadence, take its callbacks instead of sampling it on a clock. Teach recording stores one sample per feedback update, timed by the publisher's frame stamp, so there is no rate to configure. Removing repeats *after* the fact is the weaker fix and has its own cost — it leaves the survivors on the times they were taken, which is how a uniform grid became a bimodal 10/20/30 ms one
