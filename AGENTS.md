...
- **Where a source has its own cadence, take its callbacks rather than sampling it on a clock.** Teach recording stores one sample per feedback update, timed by the publisher's frame stamp, so the arm's cadence is the recording's cadence and there is no rate to configure. A clock above the source stores repeats; one below it discards updates; one at the source's rate beats against it and does both.
