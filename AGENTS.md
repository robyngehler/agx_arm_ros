...
- **The arm's feedback rate is the ceiling for every rate above it, and it is not configurable.** Complete joint state updates arrive at ~100/s on the right arm and ~137/s on the left, measured on the wire 2026-08-22. A rate configured above that — acquisition, publication, teach recording — adds no information and costs file size, dispatch time and CPU; 200 Hz recording produced 33.4% identical consecutive samples. A frame count is not an update count: one update is eleven CAN frames (four position, seven motor state), so ~2520 frames/s is ~150 updates/s. The ~2 kHz quoted for these joints is the servo loop *inside* the joint, which MIT closes locally and which never reaches CAN. Detail: `docs/sprint_refactor/reference/feedback_rate_budget.md`.
