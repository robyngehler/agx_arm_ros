...
- Treat `<side>_omnihand_controller/follow_joint_trajectory` as the production hand execution path, not a debug or development surface. Keep `control/omnihand/joint_trajectory` only as a bridge-specific compatibility surface until a later action or controller contract is finalized.
