...
- Keep combined `feedback/joint_states` as the coordinated arm-plus-end-effector feedback surface. Shared `control/joint_states` is the current hand command flow and is legacy: the V02 target is one repo-owned abstract hand command carrying the command authority stamp — `owner_id`, `device_epoch`, `unit_safety_epoch`, `sequence`. The standard ROS messages are external types and stay untouched.
