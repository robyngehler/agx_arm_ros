...
- **the arm feedback rate is measured and is the ceiling for everything above it (2026-08-22)**: ~100 complete state updates/s on the right arm, ~137/s on the left, with no protocol knob to raise it. Acquisition, publication and teach recording are currently configured at 200 Hz, above the source — a recording from that session carried 33.4% identical consecutive samples. Whether to return them to the source rate is an open decision, not a cleanup. Budget and method: `sprint_refactor/reference/feedback_rate_budget.md`
- **taught replays are time-parameterized as of 2026-08-24**, in four modes chosen per replay: `as_recorded` and `smooth` keep the taught timing, `speed_scale` and `maximize_speed` hand the path to MoveIt's time-optimal parameterization in the new `agx_arm_retiming` package. The replay message now carries velocities, so the controller has a feedforward instead of chasing position. Not wired into the coordinator yet, and three declared joint velocity limits still disagree (`open_questions.md`). Detail: `sprint_refactor/reference/trajectory_retiming.md`
- the right arm's motor-state feedback degrades along the joint chain (101/s at J1 to 63/s at J7) while the left arm is flat; about a third is contention from our own MIT stream and the rest is internal to the arm. Cause unattributed, tracked in `open_questions.md`
...
- **the held-bus classifier now reads the transmit error counter, not `tx_packets` (2026-09-03)**: an external-watchdog emergency stop could never be recognised as a hold, so every stop ran a recovery against a healthy bus and latched the lockout that refuses motion after the release. Measured on `can_nero_left`: 3589 frames left the host at ~1320/s during the hold while `tx_packets` stayed frozen, because an unacknowledged ONE-SHOT frame is never counted. Validated at L1 and by replaying the recording; **L3 hardware validation of the stop/release cycle is still outstanding**. Evidence: `sprint_refactor/reference/bus_hold_tx_evidence.md`
- **recovery could not use the session it had taken (2026-09-04)**: it quiesces the SDK worker and then submitted its own re-arm and feedback confirmation to that worker, which accepts submissions and dequeues none. The enable never reached the wire and a bus delivering ~2150 frames/s read as "not ready", so an emergency stop left the arm unusable until the stack was restarted. Both call sites now bypass the worker. Validated at L1; **L3 stop/release validation still outstanding**
