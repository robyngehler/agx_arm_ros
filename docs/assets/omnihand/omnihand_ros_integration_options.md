...
| `<side>_omnihand_controller/follow_joint_trajectory` | `control_msgs/action/FollowJointTrajectory` | Coordinated hand execution with verified delivery and, on a shared bus, the arm-quiescing window | **implemented and production.** This is the primary path for real hand motion — lower measured latency than a direct `HandCmd`, synchronizes with arm trajectories, and carries the trajectory semantics later motion primitives need. Superseded reading (before 2026-08-14): "optional later surface, add if needed" |
