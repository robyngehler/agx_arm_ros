...
| `feedback/omnihand/status` | new `agx_arm_msgs/OmniHandStatus` | Device state, temperatures, currents, control mode, fault bits, plus verified-delivery state (`command_pending`, `command_attempts`, `command_delivery_failed`) and `joint_readback_age_s` | keep implemented as the repo-owned status surface; it is the only surface that says whether a hand target actually landed, since `feedback/omnihand/joint_states` is republished from cache regardless |
...
| `<side>_omnihand_controller/follow_joint_trajectory` | `control_msgs/action/FollowJointTrajectory` | Coordinated hand execution with verified delivery and, on a shared bus, the arm-quiescing window | **implemented and production.** The primary *trajectory-execution* path — one of two production primitives, alongside reactive contact-seeking motion in the skill controller — lower measured latency than a direct `HandCmd`, synchronizes with arm trajectories, and carries the trajectory semantics later motion primitives need. Superseded reading (before 2026-08-14): "optional later surface, add if needed" |
| `control/omnihand/claim_device` | `agx_arm_msgs/srv/ClaimDevice` | Take or give up command of the hand | **required before commanding.** The bridge is fail-closed; an unclaimed hand executes nothing. Deliberately not named `claim_device`: the arm driver serves that in the same per-side namespace, and a client silently reached whichever it found first |
| `control/omnihand/stop` | `std_srvs/Trigger` | Cancel the pending hand target and hold the current pose | keep implemented as the bridge-local **cancel**. It is not a latching device stop: the hand re-arms on the next command, and only the unit generation can latch it STOPPED. Superseded reading (before 2026-08-14): "safe stop" |
