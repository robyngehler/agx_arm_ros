...
It captures confirmed repository state as of 2026-05-11, unresolved gaps, and the document set that can later be promoted into the stable top-level docs tree.

Planned stable Sprint 1 outputs from the roadmap:
...
| OmniHand assets and SDK | PARTIALLY_AVAILABLE | Vendor SDK is now vendored at `vendor/Omnihand-2025-SDK` with Python/C++ APIs, ROS2 node/message packages, and URDF/mesh assets, but it is not yet integrated into the local control/planning stack. |
...
- Sprint 1 cannot close OmniHand or AGV discovery without new repositories, vendor files, or hardware-side documentation being added to the workspace.
...
- `hand/omnihand_sdk_integration.md`: vendored SDK inventory, architecture constraint, and next wrapper decision.
