...
# SDK's own reader thread (`driver_context._read_loop`), not the driver's publish
# loop. So a stalled publisher does not stop the drain — what stops it is
# recovery calling `stop_th()` while it tears the link down, and the question is
# how many frames are lost between the bus returning and the reader restarting.
#
