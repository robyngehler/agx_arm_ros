...
- measure a CAN rate claim below the SDK with `candump` on the raw socket and cross-check `/sys/class/net/<iface>/statistics`; `candump` does not show the TX loopback, so take TX from `tx_packets`
