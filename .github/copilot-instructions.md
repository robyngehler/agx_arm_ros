...
- keep exactly one owner of a device's SDK session at any instant: steady-state calls go through that device's serialized worker on a declared priority lane, and recovery is the one exception because it takes the session off the worker. This holds for the arms today. A hand reaches its SDK from one thread only because the bridge spins single-threaded — an accident, not an invariant, and with no lane priority, so a stop waits behind ordinary work (phase 2C)
